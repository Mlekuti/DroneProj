  ...
  try {
    api.onmessage = function(msg, data)  {
      if ("event" in data) {
        ...
        // use this to display text in div container
        if (data.event == "FrontEndMessage") {
          // getting current time
          var rightnow = new Date();
          var datetime = rightnow.getHours() + ":" 
          + rightnow.getMinutes() + ":" + rightnow.getSeconds() + ": ";
          let appendhtml = document.getElementById("notification-bar").innerHTML;
          var weatherString = ""
          if (data.details.id == 0) {
            weatherString = "Sunny outside. Everything is normal";
          } 
          else if (data.details.id == 1) {
            weatherString = "Heavy rain outside. Everything is delayed at 1/2 speed";

          }
          else if (data.details.id == 2) {
            weatherString = "Heavy snow outside. Everything is delayed at 1/4 speed";

          } 
          else if (data.details.id == 3) {
            weatherString = "🚨🚨TORNADO IN AREA! ROUTE STOPPED, TAKE COVER🚨🚨";
          }  
          document.getElementById("notification-bar").innerHTML = appendhtml + datetime + weatherString + "<br>";
...
function updateWeather(weatherType) {
  // send the command to the backend for all weather types and then in transit_server make sure
  // the instance is updated to have the correct weather type and then it will create the entity
  if (weatherType == "HEAVYSNOW"){ 
    api.sendCommand("updateWeather", {
      // ATTRIBUTION: MODEL USED IS FROM "falling snow loop" (https://skfb.ly/6XIA6) by Elin is licensed under Creative Commons Attribution (http://creativecommons.org/licenses/by/4.0/).
      "type": "HEAVYSNOW",
      "name": "HEAVYSNOW",
      "mesh": "assets/model/falling_snow_loop.glb",
    ...
    // ATTRIBUTION: MODEL USED IS FROM "Sunny (2001)" (https://skfb.ly/oJWvu) by 321Blender is licensed under Creative Commons Attribution (http://creativecommons.org/licenses/by/4.0/).
    api.sendCommand("updateWeather", {
      "type": "SUNNY",
      "name": "SUNNY", 
      "mesh": "assets/model/sunny.glb",
    ...
    // ATTRIBUTION: MODEL USED IS FROM "Animated Tornado" (https://skfb.ly/6UEPA) by Yanez Designs is licensed under Creative Commons Attribution (http://creativecommons.org/licenses/by/4.0/).
    api.sendCommand("updateWeather", {
      "type": "TORNADO",
      "name": "TORNADO", 
      "mesh": "assets/model/tornado.glb",
      "position": [100, 290, 400],
    ...
    // ATTRIBUTION: MODEL USED IS FROM "cloud lightning" (https://skfb.ly/oC6oA) by Kyyy_24 is licensed under Creative Commons Attribution (http://creativecommons.org/licenses/by/4.0/).
    api.sendCommand("updateWeather", {
      "type": "HEAVYRAIN",
      "name": "HEAVYRAIN",
      "mesh": "assets/model/cloud_lightning.glb",
      "position": [-600, 1500, 0],
      "scale": [15, 15, 15],
      "rotation": [0, 0, 0, 0],
      "direction": [0, 0, 0],
...
function autoWeather() {
  let buttons = ["heavysnowbutton", "heavyrainbutton", "tornadobutton", "sunnybutton"];
  let lastIndex = -1;
  ...
  setInterval(function() {
      ...
      document.getElementById(buttons[index]).click();
      lastIndex = index;
  }, 20000); 
}
